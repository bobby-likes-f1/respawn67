describe("Account Like And Article Flow", () => {
  beforeEach(() => {
  ...
  it("logs in, likes a game, publishes an article, views it, then logs out", () => {
    const email = "btest2@gmail.com";
    const password = "Game!234";
    const unique = Date.now();
    const title = `Test Article ${unique}`;
    const content = "This is a Cypress test article content that is at least fifty characters long.";
    ...
    cy.contains("a", /Write Article/i).click({ force: true });
    cy.get("form").within(() => {
      cy.contains("label", /Title/i).parent().find("input").type(title, { force: true });
      cy.contains("label", /Content/i).parent().find("textarea").type(content, { force: true });
      cy.contains('button[type="submit"], button', /Publish Article/i).click({ force: true });
    });

    // Wait for redirect back to articles list
